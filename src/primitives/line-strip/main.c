...
qword_t *draw_line_strip(qword_t *q)
{
    int red = 255;
    int green = 0;
    int blue = 255;
    ...
    // Giftag
    PACK_GIFTAG(q, GIF_SET_TAG(6, 0, 0, 0, 0, 1), GIF_REG_AD);
    q++;
  // primitive data
    PACK_GIFTAG(q, GIF_SET_PRIM(GS_PRIM_LINE_STRIP, 0, 0, 0, 0, 0, 0, 0, 0), GIF_REG_PRIM);
    q++;
    ...
    PACK_GIFTAG(q, GIF_SET_XYZ(ftoi4( 0+OFFSET),
			       ...
			       1), GIF_REG_XYZ2);
    q++;
    PACK_GIFTAG(q, GIF_SET_XYZ(ftoi4( 210+OFFSET),
			       ftoi4(-110+OFFSET),
			       1), GIF_REG_XYZ2);

    q++;
    PACK_GIFTAG(q, GIF_SET_XYZ(ftoi4( 200+OFFSET),
			       ftoi4(240+OFFSET),
			       1), GIF_REG_XYZ2);
  
    q++;
    PACK_GIFTAG(q, GIF_SET_XYZ(ftoi4( 600+OFFSET),
			       ftoi4(600+OFFSET),
			       1), GIF_REG_XYZ2);
    q++;
    return q;
